# GENERAR UN FICHERO DE DATOS DEL GASTO SOCIAL DEL GRUP 23 EN ESPAÑA
library(dplyr)
library(stringr)



# Lectura de la tabla "tb_economica_cons.txt" extraída de la bbbdd Acces Liquidaciones 2023
tb_funcional_cons <-read.delim("BasesdeDatos/ODS1/Liquidaciones2023/tb_funcional_cons.txt", sep = ";", dec = ",", header = FALSE)
tb_inventario <-read.delim("BasesdeDatos/ODS1/Liquidaciones2023/tb_inventario.txt", sep = ";", dec = ",", header = FALSE)
...
tb_funcional_cons <- tb_funcional_cons |> 
    ...
    filter(V3== 23) |>  # Se filtra la partida 23 de Gasto social
...
desp23 <- tb_funcional_cons |> 
    inner_join(tb_inventario, by= c('Id'='Idente')) %>% 
    mutate(anyo= '2023')

writexl::write_xlsx(desp23, 'BasesdeDatos/ODS1/Gasto21-23/desp23-2023.xlsx', col_names= TRUE)
